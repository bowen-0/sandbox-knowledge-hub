...
- **Cites inline, answer forward.** State the finding and let the inline citation carry provenance — *"Pre-submission quality is the leverage [(Building Permits report, p. 22)](sources/p2-building-permits.md)"* — not a report as the subject of every sentence, and not link-list dumps at the end of paragraphs. Name a report or project in prose only where the source itself is the point: one pilot's specific outcome, a tension between named pilots, or a pattern that holds across several. When you do name one, mark what it is on first mention — *"the Smart Parking pilot report"*, *"the sandbox's Smart Parking case study"* — never a bare *"Smart Parking"*, which a reader who never sees the tool calls cannot tell apart from a generic concept. Make clear the answer is grounded in the Sandbox reports — but banned is narrating the *retrieval act* (*"I looked in the wiki"*, *"the search returned"*); framing the evidence base (*"the reports don't address this directly"*) is required (P1/P2), not retrieval-narration. Name the source generically as the *Knowledge Repository* / *the corpus* / *the reports* — never "the wiki" or "the LLM wiki".
...
- **Page-anchors substantive claims.** Use the convention from [`CONVENTIONS.md`](CONVENTIONS.md) §6: `[(<source-slug> p. N)](sources/<source-slug>.md)`, where the page number refers to the English-edition PDF. Never invent an anchor, and never write `#para-N` — paragraph anchors are a dormant spec with nothing to resolve them. The bracketed citation form is reserved for **source reports**: never bracket-cite a wiki page (a synthesis, theme, or lesson page) as provenance. Synthesis pages carry report citations on their claims — carry those through; a synthesis-level judgment with no report citation behind it is attributed in prose as the corpus's synthesis (P4), never given a page-link citation.
...
*The examples below use `[[slug]]` authoring notation to mark entity references. In a chat-client answer (the default surface) render those as plain names per the rendering rule above. Page citations stay exactly as written: the read tools serve them with a friendly source name already — `(Building Permits report, p. 22)`, the `cite_as` names carry the document kind so the citation self-describes as a report — while the link still targets the source file, so a citation always maps back to its report. (Stored markdown anchors citations by slug per [`CONVENTIONS.md`](CONVENTIONS.md) §6; the friendly name is applied at serve time. When you author a page, write the slug form.)*

**Good** (answer forward — the default; the citation carries provenance):

> Building in-house on open source is a viable procurement route when no commercial provider meets public-sector requirements for data protection, transparency, and modifiability [(Digital Eye Clinic report, p. 19)](sources/p2-digital-eye-clinic.md).

**Good** (single-pilot outcome — the source is the point, page-anchored):

> The [[digital-eye-clinic]] project explicitly demonstrated intrapreneurship in public healthcare: no commercial provider met the public hospital's requirements (data protection, transparency, modifiability), so the team built an open-source diagnostic platform in-house [(Digital Eye Clinic report, p. 19)](sources/p2-digital-eye-clinic.md).
...
**Good** (verbatim quote preserved):

> Quote the official English edition exactly as printed: *"<sentence, character-for-character from the EN PDF>"* [(Building Permits report, p. 25)](sources/p2-building-permits.md). Quotation marks are reserved for character-exact EN-edition text per [`CONVENTIONS.md`](CONVENTIONS.md) §5; never reconstruct or re-word a quote.
...
- **Don't invent anchors.** Cite at page level (`p. N`, English-edition PDF); never write `#para-N` — the paragraph-anchor pipeline is dormant and nothing resolves them.
- **Don't cite a wiki page as a source.** The `[( …, p. N)]` form always targets a source report + page. A synthesis or lesson page is a lens on the reports, not a report: carry through the report citations its claims hold, and attribute uncited synthesis-level judgments in prose. Equally, don't name a pilot bare — mark it as *"the … pilot report"* / *"case study"* on first mention so the reader knows it's a source, not a concept.
- **Don't re-word or translate quotes yourself.** Quote the official English edition character-for-character. Per CONVENTIONS §5.
