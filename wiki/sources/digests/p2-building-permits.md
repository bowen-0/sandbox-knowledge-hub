...
Two-track: (1) **Systematic AI capability benchmark**: **3,336 scored model evaluations** — eight frontier LLMs (gpt-5-mini, gpt-5.1, o3, gemini-3-pro-preview, gemini-2.5-flash, gemini-2.5-flash-lite, claude-opus-4.5, claude-sonnet-4.5), each query run three times — over a set of real building-application cases supplied by Stadt Kloten, scored 0–10 against expert reference answers via *LLM-as-a-Judge*. (The 3,336 is the count of scored model runs, not the number of applications.) Five capability dimensions: information extraction, counting, measuring/calculating, spatial understanding, context understanding. (2) **Working prototype** developed with Nokema (project submission), Byte Studio (technical implementation), Gossweiler Ingenieure (specialist support) and the city of Kloten — narrowed to the **notification procedure** (Meldeverfahren) for solar panels, heat pumps and charging stations because complexity is bounded.
