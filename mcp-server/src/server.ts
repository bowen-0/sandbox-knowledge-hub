...
const DISCIPLINE = `Answer from wiki pages, not from training data; if the wiki does not cover something, say so. Cite page-level against the source PDFs: "[(<source-slug> p. N)](sources/<source-slug>.md)". Respect insight_domain: never present a sandbox-operations insight as ai-deployment advice or vice versa. Full procedure: read the wiki://query-procedure resource or wiki_read_page "QUERY".`;
